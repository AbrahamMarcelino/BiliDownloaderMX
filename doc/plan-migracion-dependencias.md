...
| «Abrir carpeta» usa `os.startfile` (solo Windows); en Linux muestra «在Linux中暂时无法使用该功能» (no disponible) | `utils/open_folder.py` | Linux, macOS | `QtGui.QDesktopServices.openUrl(QtCore.QUrl.fromLocalFile(path))`, que funciona en los tres |
